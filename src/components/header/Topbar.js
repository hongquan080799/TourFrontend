    ...
    return (
        <div className="topbar d-flex justify-content-between align-content-center">
            ...
            <div className="topbar__utils d-flex justify-content-around">
                {state?.user != null?       
                    <p onClick={()=> history.push('/ViewAccount')}>{state?.user?.photo != null ? <img alt="pt" className="rounded-circle" style={{width:40}} src ={state?.user?.photo} /> : <i class="fas fa-user"></i>} <span>{state?.user?.displayname}</span></p>:
